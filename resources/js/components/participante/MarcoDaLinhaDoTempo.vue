...
const aparencia = computed(() => {
    switch (props.marco.estado) {
        ...
        case 'atual':
            return { icone: CircleDot, rotulo: 'Agora', circulo: 'bg-informacao text-informacao-foreground', texto: 'text-informacao-texto' };
        case 'futuro':
...
<template>
    <li class="relative flex gap-3 pb-6 last:pb-0" :data-marco="marco.chave" :data-estado="marco.estado">
        ...
        <span v-if="!ultimo" class="absolute left-[1.125rem] top-9 h-[calc(100%-2.25rem)] w-px bg-border" aria-hidden="true" />
        ...
        <div class="min-w-0 flex-1 space-y-1 pt-1">
            <div class="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                <h3 class="text-base font-semibold leading-tight">{{ marco.titulo }}</h3>
                <span class="text-xs font-medium uppercase tracking-wide" :class="aparencia.texto">{{ aparencia.rotulo }}</span>
            </div>

            <p v-if="marco.momento" class="text-sm text-muted-foreground">
                <time :datetime="marco.momento">{{ formatarDataHora(marco.momento) }}</time>
            </p>

            <p class="text-sm leading-relaxed text-muted-foreground">{{ marco.descricao }}</p>
        </div>
    </li>
</template>
